...
@interface CHistoryofConversion ()<UIScrollViewDelegate,UITableViewDataSource,UITableViewDelegate,WebserviceProtocolDelegate>
{
    CGRect mainFrame;
...
@implementation CHistoryofConversion
...
-(void)GetProviderServiceListDetails
{
    if (!IS_NETWORK_AVAILABLE())
    {
        ...
        WebserviceProtocol *Datadelegate = [[WebserviceProtocol alloc] initWithParamObject:UrlParameterString.WebParamCustomerHistoryOfConversation ValueObject:self.DataStringArray UrlParameter:UrlParameterString.URLParamCustomerHistoryOfConversation];
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void){
        ...
        if ([[ParamObjectCarrier objectForKey:@"errorcode"] intValue] == 1) {
            ...
            for (id WEbdetailsData in [ParamObjectCarrier objectForKey:@"Messages"]) {
                CustomerHistoryOfConversionList *LocalObject = [[CustomerHistoryOfConversionList alloc] initWithCHistoryOfConversionId:[WEbdetailsData objectForKey:@"id"] CHistoryOfConversionReplyCount:[WEbdetailsData objectForKey:@"reply_count"] CHistoryOfConversionCustomerId:[WEbdetailsData objectForKey:@"customer_id"] CHistoryOfConversionMessageTitle:[WEbdetailsData objectForKey:@"msg_title"] CHistoryOfConversionMessageDetails:[WEbdetailsData objectForKey:@"msg"] CHistoryOfConversionMessageTime:[WEbdetailsData objectForKey:@"msg_time"] CHistoryOfConversionIsBlocked:[WEbdetailsData objectForKey:@"is_blocked"] CHistoryOfConversionIsReplied:[WEbdetailsData objectForKey:@"is_replied"]];
                
                [self.TableDataArray addObject:LocalObject];
    
            }
            
            //NSLog(@"self.TableDataArray -- %@",self.TableDataArray);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CustomerHistoryOfConversionList *LocalObject = [self.TableDataArray objectAtIndex:indexPath.row];
    ...
    for (int i=0; i< [_HeaderContainerArray count]; i++) {
        ...
        if (i == [_HeaderContainerArray count]-1) {
            ...
            switch (i) {
                case 0:
                    [TitleLabel setText:[NSString stringWithFormat:@"%@",[LocalObject CHistoryOfConversionMessageTitle]]];
                    break;
                case 1:
                    [TitleLabel setText:[NSString stringWithFormat:@"%@",[LocalObject CHistoryOfConversionMessageDetails]]];
                    break;
                case 2:
                    [TitleLabel setText:[NSString stringWithFormat:@"%@",[LocalObject CHistoryOfConversionMessageTime]]];
                    break;
                case 3:
                    [TitleLabel setText:[NSString stringWithFormat:@"%@",[LocalObject CHistoryOfConversionReplyCount]]];
                    break;
            }
